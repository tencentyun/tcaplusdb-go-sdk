...
// value的data不超过最大长度10MB
func TestValueData10MBSucc(t *testing.T) {
	if err := cfg.ReadApiCfg("../cfg/api_cfg.xml"); err != nil {
		t.Errorf("ReadApiCfg fail %s", err.Error())
		return
	}
	...
	//使用SetValue
	if err := rec.SetValueStr("str10mb", string(make([]byte, 10*1000*1000))); err == nil {
...
//case5 value的data超过最大长度10MB
func TestValueData10MBFail(t *testing.T) {
	if err := cfg.ReadApiCfg("../cfg/api_cfg.xml"); err != nil {
		t.Errorf("ReadApiCfg fail %s", err.Error())
		return
	}
	...
	//使用SetValue
	if err := rec.SetValueBlob("byte256kb", make([]byte, 10*1000*1000+1)); err == nil {
...
// 当refer字段值大于count时报错
func TestReferLargeThanCount(t *testing.T) {
	if err := cfg.ReadApiCfg("../cfg/api_cfg.xml"); err != nil {
		t.Errorf("ReadApiCfg fail %s", err.Error())
		return
	}
	...
	if err := rec.SetData(data); err.(*terror.ErrorCode).Code != terror.ParameterInvalid {
