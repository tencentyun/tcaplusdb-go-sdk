...
func TestSyncGetFail(t *testing.T) {
...
//case2 记录存在时，Get返回成功
func TestSyncDupGetSuccess(t *testing.T) {
	if err := cfg.ReadApiCfg("../cfg/api_cfg.xml"); err != nil {
		t.Errorf("ReadApiCfg fail %s", err.Error())
		return
	}
	...
	////2 Get记录已存在
	req2, err := client.NewRequest(cfg.ApiConfig.ZoneId, TestTableName, cmd.TcaplusApiGetReq)
	if err != nil {
		t.Errorf("NewRequest fail, %s", err.Error())
		return
	}
...
//case3 Get错误的key字段，失败
func TestSyncGetErrKeyFail(t *testing.T) {
	if err := cfg.ReadApiCfg("../cfg/api_cfg.xml"); err != nil {
		t.Errorf("ReadApiCfg fail %s", err.Error())
		return
	}
...
//case4 Get错误value字段，失败
func TestSyncGetErrValueFail(t *testing.T) {
	if err := cfg.ReadApiCfg("../cfg/api_cfg.xml"); err != nil {
		t.Errorf("ReadApiCfg fail %s", err.Error())
		return
	}
	...
	if err := client.SendRequest(req0); err != nil {
		t.Errorf("SendRequest fail, %s", err.Error())
		return
	}

	//recv resp
	resp0, err := recvResponse(client)
