...
func getTtlExample() {
	//创建请求
	req, err := client.NewRequest(ZoneId, TableName, cmd.TcaplusApiGetTtlReq)
	if err != nil {
	...
	for _, resp := range respList {
		...
		var ttl uint64
		record.GetTTL(&ttl)
		fmt.Printf("response record ttl %d data %+v, route: %s\n",
			ttl, data, string(data.Routeinfo[0:data.Routeinfo_Len]))
	}
